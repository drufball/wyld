...
- **Not placed in M2 — `coil` (Kelpmaw) and `shard` (Glasswing):** `placeTracks` places only standing species plus Pyreclaw's rim exception, so these two are absent at every seed (measured 0 at seeds 194, 7, 212, 1, 999, and 31337 while every placed species got 4–8). This is intentional per `wyld-spec.md` §11 and Kelpmaw's “none in the open; inside the grotto” line; Kelpmaw's coil arrives with the M5 grotto. Glasswing's spec entry has no “none in the open” caveat, so whether its shard should be placed on the Salt Flats is an open design question, deliberately left unchanged here.
