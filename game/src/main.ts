...
import { resolveTaps, type TapBody } from './input/resolve-taps.js';
...
const makeBodyPicker = (bodies: readonly TapBody[]) => {
  if (look === 'diorama') {
    const candidates = bodies.map(({ key, tileX, tileY, tier }) => ({
      key,
      tileX,
      tileY,
      heightTiles: tier === null ? 1 : render3d!.TIER_LENGTH_TILES[tier] * 0.6,
      widthTiles: tier === null ? 0.6 : render3d!.TIER_LENGTH_TILES[tier] * 0.6,
    }));
    return (clientX: number, clientY: number) =>
      dioramaView!.pickBody(clientX, clientY, candidates);
  }
  const candidates = bodies.map(({ key, tileX, tileY, tier }) => ({
    key,
    tileX: tileX - player.screen.x * view.cols,
    tileY: tileY - player.screen.y * view.rows,
    sizePx: tier === null ? 24 : tier === 1 ? 16 : tier === 2 ? 24 : 32,
  }));
  return (clientX: number, clientY: number) => flatView!.pickBody(clientX, clientY, candidates);
};
const registryWorld = {
  regions,
  ...
  update: (dt) => {
    ...
    if (queuedTaps.length > 0) {
      ...
      const intents = resolveTaps({
        ...
        selection: partyState.selection,
        party: partyState.party.map(({ individual }) => individual),
        partyTile: (id) => partyControllers.get(id)!.tile,
        combatantOf: (id) => combat?.party.find((member) => member.id === id) ?? null,
        playerTile: player.tile,
        listWild: () => registry.list(),
        tierOf: (speciesId) => speciesById(speciesId)!.tier,
        tileOnScreen: (tx, ty) => tileOnScreen(tx, ty, 0),
        makeBodyPicker,
        pickTile: (clientX, clientY) => view.pickTile(clientX, clientY),
      });
      for (const intent of intents) {
        if (intent.kind === 'select-creature') partyState = selectCreature(partyState, intent.id);
        else if (intent.kind === 'select-player') partyState = selectPlayer(partyState);
        else if (intent.kind === 'target-wild')
          partyState = targetWildCreature(partyState, {
            id: intent.id,
            speciesId: intent.speciesId,
          });
        else if (intent.kind === 'clear-target') partyState = groundTapped(partyState);
        else if (intent.kind === 'walk-player') player.tap(intent.clientX, intent.clientY);
        else if (encounter?.state().phase === 'fight') {
          const combatState = encounter.state();
          const result = encounter.hear(intent.creatureId);
          const entry: OrderEntry = {
          ...
          orders.record(entry);
          if (entry.heard) {
            encounter.override(intent.creatureId);
            partyControllers.get(intent.creatureId)?.tap(intent.clientX, intent.clientY);
