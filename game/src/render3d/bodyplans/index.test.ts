...
describe('body plans', () => {
  it('maps every species through exactly eight builders', () => {
    ...
    for (const data of species()) {
      const built = buildBodyPlan(data, data.tier);
      let meshes = 0;
  ...
  it('hovers and floats', () => {
    const birdData = speciesById('thornwren')!;
    const bird = buildBodyPlan(birdData, birdData.tier);
    bird.animate(0, 'idle');
    const idle = bird.group.children[0]!.children[0]!.position.y;
    ...
    const ground = swimmer.group.children[0]!.children[0]!.position.y;
    swimmer.animate(1, 'idle', 1.5);
    expect(swimmer.group.children[0]!.children[0]!.position.y).toBeGreaterThan(ground);
    bird.dispose();
    swimmer.dispose();
  });
  it('disposes each resource once and tolerates a second call', () => {
    const data = species()[0]!;
    const built = buildBodyPlan(data, data.tier);
  ...
  it('builds a model for every species in the roster', () => {
    for (const data of species()) {
      const built = buildBodyPlan(data, data.tier);
      expect(built.group.children.length, data.id).toBeGreaterThan(0);
      built.dispose();
    }
  });
  it('sizes a model by its tier, not by its metres', () => {
    for (const tier of [1, 2, 3] as const) {
      const data = species().find((entry) => entry.tier === tier)!;
      const built = buildBodyPlan(data, tier);
      // Body-plan dimensions are authored in metres. The outer uniform scale converts the
      // declared longitudinal extent to the tier's readable tile length without squashing it.
      const length = built.group.children[0]!.scale.z * data.visual.length!;
      expect(length, `${data.id} should use the tier ${tier} target length`).toBeCloseTo(
        TIER_LENGTH_TILES[tier],
        1,
      );
      expect(length).toBeGreaterThanOrEqual(TIER_LENGTH_TILES[tier] * 0.9);
      expect(length).toBeLessThanOrEqual(TIER_LENGTH_TILES[tier] * 1.1);
      built.dispose();
    }
  });
});
