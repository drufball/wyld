...
const createToastStack = () => {
  ...
  const display = (titleText: string, hintText?: string): void => {
  ...
  const show = (discovery: Discovery): void => display(discovery.title, discovery.hint);
  const note = (text: string): void => display(text);
  return { root, show, note };
};
