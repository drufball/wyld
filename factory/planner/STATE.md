...
| 1.8 Paused (quest `paused`) | **in progress — pulled forward 2026-09-06 07:22** after Dru asked about the ntfy card. Lead spawned by the Planner: unit A = self-hosted ntfy (`.factory/bin/ntfy`, tmux window `ntfy`, Tailscale Serve on a second HTTPS port `:8443`, server `notify()` + `POST /api/notify` + Wake `pak_notify`, then re-file Rumble `ntfy-phone` with the real address), unit B = pause/resume state + `pak_pause`/`pak_resume` + Wake buffering + ops watchdog. **Unit C (Pak banner + Resume button + VMU) is NOT started — it waits for `pak-theme` to finish, then needs its own lead.** Runs in parallel with the `pak-theme` lead (disjoint packages) | **#96 / PR #98 landed 2026-09-06 07:44** — the transport-agnostic half of unit A: `factory/server/src/notify.ts` `createNotifier()` (fire-and-forget, copies `createWakeForwarder`'s shape), `POST /api/notify {title,message,tags?,click?}` → 202 `{sent}`, `NTFY_URL`/`NTFY_TOPIC` config (empty `NTFY_URL` = silent no-op, one startup warning — same discipline as `WAKE_URL`), Wake `pak_notify` (schema in `channel.ts`, hot-reloads), doctor warns when `NTFY_URL` is unset. Verified end-to-end against a real ntfy server: title/message/tags/click all arrive. No fix rounds. **Self-hosting ntfy on this laptop is impossible** — see the sharp edge below; Rumble `ntfy-phone` now asks Dru to choose the transport and unit A cannot finish until he taps. **#99 / PR #101 landed 2026-09-06 08:0x (unit B1)** — `pauses` table (lane/reason/fix/since/resolved_at/rumble_id, migration 0009), `POST /api/pause` (idempotent per lane: a repeat returns the existing row with 200 and emits no second event, no second rumble write, no second push) and `POST /api/resume {lane?}`, `system.paused`/`system.resumed` (payload carries `reason` as a string, which is what `catchup.ts` renders), an outage Rumble `outage-<lane>` whose single `Resume` option resumes **server-side** so the button works while the Planner is down, and `paused` on `GET /api/health/snapshot`. `writeRumble` was extracted from `rumbles.ts` and is now shared. One fix round, for two real defects — see the sharp edge below. **Dru chose "Run it privately in Docker" for the ntfy transport (08:12); Dru chose "Run it privately in Docker" (08:12) and the transport landed the same hour.** **#103 / PR #105** — ops quota watchdog in the existing 2-min cycle: hysteretic GitHub thresholds (`OPS_GH_RATE_PAUSE_BELOW` 200 / `OPS_GH_RATE_RESUME_ABOVE` 500, deliberately apart so a limit hovering at the boundary cannot flap the factory), `OPS_PLANNER_STALE_MINUTES` 15, `OPS_WATCHDOG` to disable. Codex is **deliberately not auto-detected** (ops neither runs Codex nor sees its quota errors, and "no PR in N minutes" is indistinguishable from a big task) — the Planner reports it via `pak_pause`. Verified live: it paused both lanes with correct plain-English cards and resumed them. **#104 / PR #107** — the Pak's calm banner in `Shell()` (`Card variant="bevel" data-tone="bad"`, pixel font on the `Paused` label only), a 44px Resume button, the outage Rumble already red-topped from the theme work, and `Paused?` as the VMU's first row. Verified in a real browser at 375x812 and 1280x900: zero horizontal overflow, Resume clears the banner and marks the outage card decided. **#106 / PR #109** — the private ntfy container. **#102 / PR #108** — Wake holds events while paused: a persisted `wake_state` row, secret-gated `POST /pause`/`/resume` on Wake that the Pak server calls fire-and-forget, `paused` on Wake's `/health`, and the `pak_pause`/`pak_resume` tools. Buffering needs **no replay code**: pending rows are simply left with `deliveredAt IS NULL`, so the ordinary next claim returns them in `ts` order. One fix round, for the lane bug below. `factory:doctor` is **30 ok / 0 warn / 0 fail**. Step 1.8 is complete. |
