...
- A **quest** is one user-relevant feature Dru would notice, closed **within the day** — the normal rhythm is a few quests closed each day (Dru's scale, 2026-09-09: "Quests should rarely be days… you usually close a few quests each day"). Its pitch carries a one-line "done when…". If a quest keeps receiving new units after it reached `demo`, it has become a mission: leave it as shipped and split the new work into new quests.
- A **mission** is a couple-of-days sprint to a milestone outcome — a block on the roadmap, not a quest: what will be true, the quests under it (with their state), the live playthrough embedded (`data-wyld-demo` iframe), and the mockups for what's next. Dru's feedback on a mission lands on the roadmap or on a quest's explainer, never on a bucket quest.
