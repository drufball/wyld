...
| `sleep.alarm` | The night-shift clock (1.10): `goodnight` (Dru pressed GOODNIGHT, or 23:00 came), `last_call` (07:15), `lights_on` (08:00). Payload carries `runId`, `alarm`, `trigger`, `lightsOnAt`. | Run `skills/sleep-mode.md` from that alarm: `goodnight` → start the Drain phase; `last_call` → finish whatever phase you are in, write the retro, do Reset; `lights_on` → end the run now (`pak_end_sleep`) and go quiet. If the server already ended it (its 08:00 guard), just make sure the retro is yours, not the mechanical stub. **Never end the run at Reset**: after Reset, leave it open and wait for `lights_on` — that alarm is the Planner's only guaranteed morning event (the host wakes on events; session cron does not fire for it), and it does not fire for an already-ended run. 2026-09-08 was lost to ending the night early. **A finished lead does not wake the host** — its report is delivered only at the next turn, and turns come from channel events. 2026-09-08 night: the Sweep finished 23:32 local, the report arrived with `last_call` at 07:15, QA/Retro/Reset ran with 45 minutes to spare. When a night lead is spawned, schedule a wake (`CronCreate`, one-shot, ~10 min after the lead's expected finish, and again if it hasn't reported) so the phases advance without waiting on luck. **Ending the run:** the server ends it itself at `lights_on` and records `timed_out` (run 3, 2026-09-09, though every phase was done). **Corrected 2026-09-10 after a second lost day:** ending the run in the last-call window meant `lights_on` never fired (the alarm belongs to the run) and the host slept until 21:52. So: **never call `pak_end_sleep` before `lights_on`**; let the server end the run at lights_on (a server fix makes that `clean` when reset was reached), and **always hold a session cron at 08:03 local as the morning wake regardless of the run** — the host only turns on events, and a missed alarm is a lost day. The ops health pulse keeps the Debug Menu green while the host isn't turning: it is not evidence the Planner is working. **The briefing carries no questions:** it is read and dismissed, nothing in it can be answered. Anything that needs Dru's call is its own `question` chain (`pak_send_message kind=question`), and the briefing may at most point at it (Dru, 2026-09-09: a design question buried in FYI had no way to be answered). **Quests are contained; missions are roadmap blocks** (Dru, 2026-09-09): a quest is one user-relevant feature closed within the day (a few per day is the rhythm), with a stated "done when"; a mission is a couple-of-days sprint to a milestone; work that outgrows it becomes a mission on the roadmap (with the live playthrough and mockups embedded) split into new quests — never a bucket quest that keeps taking units. See plan-quest.md. |
| `planner.tick` | The server's hourly heartbeat (2026-09-10, Dru: "a deterministic cron wake event"), delivered through Wake like any event whether or not a night is open. Landed in #302. | The guaranteed turn. Send a health heartbeat; `ListAgents` and act on any lead report that arrived; read `.factory/planner/STATE.md` for the next planned step and take it if its trigger has passed (a morning with no `lights_on` seen → start the day's leads); never write to the Pak for the tick's own sake. Session crons are a backstop, not the mechanism. |
