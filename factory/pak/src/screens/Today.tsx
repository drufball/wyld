...
export function Today({
  ...
  const composerButton = useRef<HTMLDivElement>(null);
  ...
  useEffect(() => {
    const escape = (event: KeyboardEvent) => {
      if (event.key === 'Escape' && composerOpen) {
        ...
        composerButton.current?.querySelector('button')?.focus();
      }
    };
  ...
  return (
    <div className="grid gap-8">
      ...
      <Card
        ...
        className={
          composerOpen
            ? 'fixed bottom-[calc(88px+env(safe-area-inset-bottom))] left-3 right-3 z-40 p-5 md:bottom-24 md:left-auto md:right-6 md:w-[min(440px,calc(100vw-3rem))]'
            : 'sr-only'
        }
      >
