...
describe('readTokensToday', () => {
  it('streams valid entries from the current UTC day', async () => {
    const directory = await mkdtemp(path.join(os.tmpdir(), 'wyld-ops-'));
    ...
    const line = (timestamp: string, model = 'claude') =>
      JSON.stringify({ type: 'assistant', timestamp, message: { model, usage } });
    await writeFile(
      path.join(directory, 'one.jsonl'),
      [
        line('2026-09-05T01:00:00Z'),
        line('2026-09-04T23:59:59Z'),
        '{bad',
        line('2026-09-05T02:00:00Z', '<synthetic>'),
