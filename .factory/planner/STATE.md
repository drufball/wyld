...
- 17:00 Dru (chain 95): saw "a new creature" in the M0 disc; verified discs are isolated (`demosDir/<slug>`, exact sha, hashed assets; M0 bundle has zero creature/species/wander strings). Asked for a feedback screenshot if it recurs. His design point → quest `demo-scenarios` (planning, world fieldwork): game opens on `?scenario=<id>` (start, phase, spawns, one-line goal); each disc's deep link points at its scenario; retrofit M0–M2 discs. Brief to write; runs in `game/` after M2 + the view decision. Consider making scenarios part of every milestone lead's §3 disc registration from M3 on.
...
- **M2 lead edges (17:40):** `pak_register_demo` summary ≤ 400 chars; debug-spawned creatures never call (`spawn` doesn't set `nextCalls`) → 2D unit 2 fixes; headless settle identifies everything in view (`face` far first, then `tp`); new debug cmds `tp x z`, `tracks`, `face`, `reveal guide|map`; Codex verification section false on 3/4 units (typecheck ×2, invented flaky test ×1); VERIFICATION M2 lacks Planner-review lines → 2D lead folds the six measured lines into unit 1. Map takes an injected `WorldSampler`; fog pure; tracks decals merged per species (draw calls +10 not +40). Hints validator: phase names case-sensitive whole words. Final disc `9d2c138`, 9 steps.
- **Tonight's Sleep Mode (23:00) prep:** Drain — the 2D lead leaves unit 2 as a draft if not clean by 22:30; #218 and the ⌘-pin follow-on must be merged or draft. Sweep candidates: server emits `planner.chain_updated` on unlock-chain creation + drop the Today workaround; `spawn` sets `nextCalls` (if 2D unit 2 hasn't); mechanical Catch-Up link rule (if #218 didn't cover it). Ash Fields density is moot under 2D — drop. QA: `pnpm smoke`, discs ready (fw-m0/m1/m2 + fw-2d), don't re-register hidden Main. Retro: per-PR cycle time, Codex false-green count (5 today), the three decisions. Reset: Catch-Up, next action, end run.

## MISSION TIGHTENED 17:40 — "a core combat loop that's fun" (chain 100, pinned on the roadmap mission)

- Dru: roster of 6, 3 enemies; pick enemy → pick 3 → mini arena; goals: fluid movement, learning as the core of fighting (live/die/repeat, facts into the guide, pick counters).
- 2D lead told to STOP: close #220 superseded, close its PR unmerged, report. `fw-2d` parked (2D decision stands, applied to the arena). M3 folded into `fw-arena`.
- Quest `fw-arena` (planning) — brief `/tmp/wyld-leads/fw-arena/brief.md`: separate entry `arena.html` (3D game untouched), 3 units: arena+movement+sprites (60 Hz sim + interpolation, p95 < 16.7 ms); the fight (§7 maths, AI, win/driven off); the loop (pick screens, guide facts from hits, persistence). Roster/enemies suggested so each enemy's weakness is covered by two roster creatures. Defaults told to Dru: player in the arena, caught = loss; no sneaking v1.
- Spawn the arena lead once the 2D lead reports (same package) — minutes, not hours.
- Roadmap v14: new mission block (two crossed-off predecessors), arena the only quest in Now, workshop + "depth" in Next, the world loop in Later.
- **17:43 REVERSAL:** Dru keeps the whole-world 2D rebuild first ("everything builds on that style"), arena after. 2D lead told to resume (reopen #220/PR if closed) + add an `arena` scenario stub in unit 3. `fw-2d` building again. Arena brief to shrink before spawn: unit 1 = arena place + pick screens (sprites/movement/party come from fw-2d); units 2–3 as written. Roadmap v15. #221 (⌘-pin + clipping) filed by the Pak lead.
- Dru marked `fw-m2-guide` done (chain 86).
- **18:02 #222 review (2D unit 1):** Codex shipped 9 of 23 named tests, stubbed `reveal guide`, **deleted the event bus + event log + `phaseChanged`**, dropped unrelated guide/stats code, and drew cover tiles as grass. Lead caught all of it headless; one big fix round. Rule for briefs: when saying "delete Three-only modules", **name what must survive** (`engine/events.ts`, `reveal guide`, the debug panel) with a grep/behaviour test, not just "what nothing imports". Sprite blit = one `fillRect` per pixel (165 draw calls per sprite) → cache to offscreen canvas; the draw-call rule again.
- 18:03 #218 deployed and verified live (player iframe `/play/<id>/?debug=1`; `/demos/one-mechanism` → Today); told Dru. **#223 (⌘-pin) changes `PIN_BRIDGE_SNIPPET`** (adds `wyld:pin:key` held/released, blur/visibility release) → after its deploy, republish all five explainers: `cd factory/server && env -u … npx tsx /tmp/wyld-leads/artifacts/publish.mjs …/pin-bridge.ts --force` (no `ONLY`).
- **18:35 2D unit 1 merged (#222)** — 3 rounds, 57 min. Final state per lead: 23/23 tests, Three-free 67 kB bundle, event bus restored, draw calls 167→2 (offscreen sprite cache), viewport 20×15@3× desktop / 11×22@2× phone, cover visible, water blue, tap on unwalkable → nearest walkable neighbour, edge flip searches along the edge for a clear crossing, phone HUD shrunk. Unit 2 (sprites/creatures/sneaking/tracks) next — biggest unit; 22:30 draft rule applies.
- 18:45 rerun-rule refinement: the Pak-only-flake → rerun rule applies to **game** PRs. On a Pak PR, a flaky test **in the area the PR changes** (here `e2e/explain.spec.ts` on the ⌘-pin PR) is a round: loop it ≥ 10× under load against the branch; merge only on 10/10. Told the Pak lead before #223 merges.
- 18:46 #223 merged (on a rerun — lead told to loop the pin e2e ≥10× on main post-deploy; failure → follow-up issue) and deployed; verified the bundle handles `wyld:pin:key`; **all five explainers republished `--force`** (artifacts v7, fw-m0 v3, fw-m1 v2, fw-m2 v2, roadmap v16); told Dru. Snippet now 3267 chars.
- **Pak lead final (18:50):** #218 + #223 live and browser-verified. Pin e2e red was **pre-existing** (2/8 on `main` under load; test asserts the API count before the auto-waiting UI assertion; no `wyld:pin:key` traffic in that test) → **Sweep tonight: deflake `factory/pak/e2e/explain.spec.ts:40`** (move the `Pinned comment 1` visibility assertion above the API assertions). Also Sweep: `Demos.tsx` disc player hardcodes `bottom-[72px]` → same phone safe-area clipping as the explainer had (measure, then fix like #223). Edges: #216's raw-JSON premise was an older build — the Catch-Up generator was the real bug; `ChainList` "How to try it" for non-disc cards now lands on `/demos` (weaker; fine); **jsdom can't see layout — viewer/layout work needs headless Chromium in the review**.
- Workshop brief pre-written at `/tmp/wyld-leads/workshop/brief.md` (3 units: view / edit+create / ship-as-PR; sprite generator must be importable — else unit 1 moves it to `@wyld/sprites`). Spawn the Pak lead when 2D unit 2 merges (fill in the generator's export/spec type first).
- **2D lead unit-1 report (18:55):** the lead *stops at unit boundaries* and needs `SendMessage` to resume — told it to run through unit 3 or 22:30 without stopping. Edges: `three`/`@types/three` still in `game/package.json` (unit 3 removes); guide silhouette placeholder until unit 2 wires `generateSprite`; `getState()` lost `camera`/`stance`, gained `screen`/`tile`; `ai.ts` still has `stance` (unit 2 removes); 2D harness `/tmp/wyld-probe2d.mjs` (`VW`/`VH`/`TOUCH`); prop densities raised ~10× (conifer 1.3→18 etc.) so forests read at 1 tile = 2 m; spec resolutions in VERIFICATION (frustum → same screen + tile LOS ≤ 20; occluded → off-screen; opening beat deferred to M6). Unit 2 PR #225 open; sprites dir verified DOM-free.
- 19:13 #225 round 2: sprites now read (three facings, anchored), but **`blocksSight` on every cover tile + the raised density = nobody ever sees you** (Thornwren meter 0.000 over 14 s). Fix: only trees/cliffs block sight, rocks block walking only; conifer 18→10, broadleaf 12→7 (BALANCE.md). Lesson: readability tuning and stealth tuning pull opposite ways in 2D — measure detection after any density change. Also: debug `spawn` breached the 24 cap (25) → enforce in the registry.
- 19:23 #225 round 3→4: **M1's detection fill was 0.12/s vs §7.7's 0.6/s (drain 0.3)** — it passed M1's checkpoint only because 3D vision was continuous; with the 2D 120° cone the meter peaked at 0.55 and drained. Retro item: a checkpoint can pass on a wrong constant; leads should assert spec numbers directly, not just behaviour. Also: hearing now turns the creature toward the player before the vision check.
- **19:44 2D unit 2 merged (#225)** — 4 rounds, 66 min: sprites (3 facings, anchored, `visual`-driven proportions), 2D detection to spec numbers (0.6 fill / 0.3 drain, hearing turns the creature), rocks block walking not sight, forest 10/7, cap enforced in the registry, seeded calls. **Workshop lead spawned 19:45** (Pak lane; unit 1 lifts `sprites/` to `@wyld/sprites` — the one game-side touch; told to keep it a pure move and rebase). Game lane: 2D unit 3 next → `fw-2d` complete → arena lead.
...
- **2D lead final (20:25):** disc `8de63aa` ready, quest demo. Edges: creature sprites drawn ~4 px left of centre and one tile high (`main.ts` `drawCreatureSprite` offsets) and tracks decals read as specks → **tonight's Sweep** (game/, tiny, after the arena's unit 1 is in or drafted); scenario ids `world|creatures|guide|party|arena`; `observe.ts` `creatureExecutedMove` placeholder is the combat seam; harnesses `/tmp/wyld-probe2d.mjs`, `/tmp/wyld-probe2ds.mjs`. Brief corrections: desktop viewport 20×15@3×; detection 0.6/0.3 must not drift. fw-2d explainer v1 published; roadmap v17.
- 20:48 **My arena brief's roster rule was inconsistent** ("exactly two" + a roster with one Heat source) → Codex dropped Barrow for Ashcrawl. Corrected rule sent to the lead + brief: Barrow always on the roster; ≥ 2 counters per enemy; every roster creature neutral-or-better vs one enemy; fix via enemies (Tidewhelk as a Shell enemy makes Loamox's Impact matter). Retro: sanity-check a brief's data rule against the data before filing.
- 21:48 **Pak down ~1 min after #229 merged**: `tsx watch` restarted the server into `ERR_MODULE_NOT_FOUND` (new `@wyld/sprites` package not yet installed/linked) until the lead's deploy touched `main.ts`. Rule for merge-and-ship: when a PR adds a workspace package, `pnpm install` **before** anything restarts the server; add to `merge-and-ship.md` tonight (Sweep, planner-owned).
- **21:52 local — clock correction:** channel/Pak timestamps are **UTC**; local is UTC+1. I'd been quoting UTC as local to leads (they use `date`, so their cutoffs were right). Sleep Mode 23:00 local = 22:00Z; the 22:30 cutoff = 21:30Z. Told the workshop lead: no unit 2 tonight; the arena lead: 40 min to cutoff, draft if not clean.

## Sleep Mode plan for tonight (written 21:55 local)

- **Drain:** at the alarm, `gh pr list --state open --json number,isDraft,title` — anything non-draft from the arena/workshop leads → ask the lead to draft it (or draft it myself if the lead has ended). Leads must have reported and ended.
- **Sweep:** one short-lived Codex lead (opus), three tiny issues, sequential, all `<!-- quest:… -->` tagged to their quest: (a) `game/`: `spawn` sets `nextCalls` via the seeded call stream if still unset; creature sprites drawn 4 px left / one tile high in `main.ts` `drawCreatureSprite` — centre and ground them; tracks decals sized to read as footprints (quest `fw-2d`); (b) `factory/pak`: disc player `Demos.tsx` `bottom-[72px]` → safe-area-aware like the explainer fix in #223, verified headless at 375×812 (quest `one-mechanism`); (c) server emits `planner.chain_updated` on unlock-chain creation + drop the Today workaround (quest `one-mechanism`) — skip if not trivial. Planner-owned: `merge-and-ship.md` gets "`pnpm install` before any server restart when a PR adds a workspace package" and "leads report at unit boundaries **and keep going**"; briefs template line "run typecheck/lint/test yourself".
- **QA:** `pnpm smoke` (deflaked pin e2e), `/api/health`, discs `fw-2d` + `fw-m0/m1/m2` ready, `/workshop` renders 13 sprites, explainers serve with the `wyld:pin:key` snippet; don't re-register hidden Main.
- **Retro:** `gh pr list --state merged --search "merged:>=2026-09-07" --json number,title,createdAt,mergedAt` → per-PR cycle time; ~19 PRs today; Codex false-green ×6; Dru's calls: tap-to-move/no stances/party of three; 2D Link's Awakening; mission → arena, then "rebuild first"; my slips: `/demos/<slug>` links for live cards, the roster rule, UTC-as-local, stopping the 2D lead for 3 minutes; the Pak minute-outage; leads stop at boundaries. What went well: lead reviews caught vacuous tests, deleted event bus, invisible cover, sight density, detection constants — none of which CI saw.
- **Reset:** Catch-Up over the night range; next action = the arena's pick screens if #231 merged, else the finished 2D disc; `pak_end_sleep`.
- **Workshop lead unit-1 final (21:58 local):** #229 merged `31195a1`, Species tab live at `/workshop` (nav `SPECS`/`Species`, measured: 5-char cap at 375 px), `@wyld/sprites` package with `lib: ["ES2023"]` (DOM = typecheck error), `GET /api/species`, pin e2e deflake shipped 10/10. Edges: deploy order `pull → pnpm install → build → touch main.ts` and **check `factory/server/node_modules/@wyld/` symlink** before restart; scratch `.mjs` never inside a package dir (phantom lint errors) — use `node_modules/.name.mjs`; headless probes of the live Pak must `page.route('**/api/catchup')` → `{show:false}` (never dismiss Dru's real one); unit 2 consolidates `validateSpecies` + rule tables into `packages/sprites` (enums duplicated on purpose for now). Resume tomorrow with `SendMessage` (unit 2: edit + create).
- **Arena lead unit-1 report (22:08 local):** #231 **draft**, review of 12 items standing, Codex fix round still running (push lands overnight; re-verify in the morning). p95 8.9/10.0 ms. Enemies now Antlerback/Glasswing/Saltwing; roster Barrow, Quill, Pip, Sedge, Cinder, Grit. Brief errors fixed: deep link is `/?scenario=arena` (no `arena.html`); "arrive within 0.25 tile" is arrival-only (Codex made it a per-step floor → quarter-tile snapping everywhere, incl. world scenarios — **blocking**). Other findings: `getState().arena` stuck on `pick-enemy`; pick keydown listener survives the fight; observer/fog run in the arena; Fight/Back below the fold at 375 px. Worktree `wt1` kept for morning. **All three leads have ended; nothing non-draft open.** Resume: arena (`SendMessage`), workshop unit 2 (`SendMessage`), 2D lead not needed.

## Sleep Mode run 2 (2026-09-07 23:00 local)

- Drain dry (only #231, draft; no Codex tasks; leads ended). Sweep lead spawned 23:01 from `/tmp/wyld-leads/sweep-0907/brief.md` (2D sprite offset + tracks size + `spawn` nextCalls; disc player inset; unlock chain event). Then QA (`/tmp/wyld-leads/sweep-0907/qa.cjs` + `pnpm smoke`), Retro (draft at `/tmp/wyld-leads/sweep-0907/retro-draft.md`; 27 PRs, mean 26 min), Reset. No STATE pushes until the Sweep's merges are through (docs runs cancel `main` runs).
- 23:30 **Disc builds broken since #229**: `builder.ts` installs then runs `pnpm --filter @wyld/game build` alone; `@wyld/sprites` resolves to unbuilt `dist/` → rollup resolve failure (`fw-2d` disc at `e0d2514` failed). Fix = `--filter "@wyld/game..."` (deps first) for both targets → Sweep 2b handed to the Sweep lead ahead of Sweep 3; re-register `fw-2d` at `e0d2514` after the server deploys. Rule: any new workspace package must be checked against the disc builder the same day.
- 00:07 Sweep so far: #233 (sprites grounded, tracks sized) 1 round; #235 (disc player inset; caught a desktop containing-block regression headless) 1 round; #237 (builder: `@wyld/game...` — the pak target already had `...`) 1 round, server restarted 23:06Z. `fw-2d` disc re-registered at `e0d2514` — awaiting `ready`. Sweep 3 (unlock event) next if time.
- 00:13 Sweep lead **died on an API ENOTFOUND** after filing #238; Codex had already opened PR #239. Resumed the lead via `SendMessage` (context intact). Rule: a lead that dies mid-task is resumed, not respawned; check for orphan PRs first.
- **Sweep final (00:25 local):** #233 (0 rounds), #235 (1), #237 (0), #239 (0) — all deployed; `fw-2d` disc ready at `e0d2514` (re-registered by me 00:07, builder fix verified). Edges: **game tile space is continuous** (tile i centre = i+0.5; pixel = t×16; bottom edge = t×16+8) — my "4 px left / one tile high" was wrong (4 px right / 8 px up); state the invariant, not constants. **`md:relative` not `md:static`** for panels with absolute chrome. **vitest 5 has no `--repeat`** — shell loop under `yes >/dev/null`. Playwright `page.route`: register the catch-all FIRST, specifics LAST. `git pull --rebase` when local STATE commits exist. Sweep 1(c) was already done (`spawn` seeds `nextCalls`). `coil`/`shard` decals never placed at seed 194 — eyeball on another seed. Codex false-green #7 (invented `tiles.test.ts` timeout). Harnesses: `/tmp/wyld-leads/sweep-0907/probe-pak*.mjs` (static Pak + stubbed API), sprite placement by palette-colour scan.
- **Sleep run 2 ended clean 00:25 local.** Retro written (27 PRs, mean 26 min). Leftover parked: #231 draft. Catch-Up 1753→1790. Morning: resume arena lead (`SendMessage`) on #231's final head; resume workshop lead for unit 2; roadmap after each.

## 2026-09-08 — A LOST DAY (19:15 local)

- **The 07:27 cron never fired** (session cron fires only when the REPL idles; this host wakes on events). No events 23:23Z→18:10Z; no builds all day; background leads gone. Dru opened the Pak at 19:10 and asked about the Catch-Up screen. Told him plainly.
- **Rule (PROTOCOL-level): never end a Sleep run at Reset — end it when `lights_on` fires** (a real event; it doesn't fire for an ended run). Never rely on session cron for a wake. Longer term: a Wake-scheduled `planner.wakeup` event (quest idea).
- Re-spawned: arena lead (resume from #231 draft head b526b0c → merge → unit 2/3), workshop lead (unit 2 → 3). Brief written for `catchup-chain` (Dru's ask: briefing as a pinned chain, dismiss = read, pin any chain; bug = the Catch-Up gate bouncing its own links) at `/tmp/wyld-leads/catchup/brief.md` — spawn next turn (3rd lead; Pak lane, disjoint files from the workshop).
- 19:15 Dru (fw-2d feedback): the disc player's Feedback button overlaps the game's bottom-right HUD → handed to the catch-up-chain lead as a small third issue (`codex/player-chrome-bar`, quest fw-2d): Back + Feedback in a slim player bar inside the measured panel; iframe gets the rest. Roadmap v18 (briefing quest in Now). Dead cron deleted. Hold STATE pushes while three leads merge.
- 19:17 Dru (fw-2d feedback): "right direction but flat — plays 2D, looks 3D at an isometric tilt, like the Link's Awakening remake." Rumble `look-diorama` (taste): (1) real tilted-ortho 3D view over the tile world reusing M1 body plans from history (~1 quest, 3 units — recommended); (2) fake depth on canvas (1–2 units); (3) stay flat. Quest `fw-diorama` (planning, blocked). Told the arena lead: game logic rendering-agnostic from unit 2. If (1): brief = Three.js back as *view only* (ortho camera ~35° tilt, tiles extruded, trees/rocks as chunky meshes, creatures = M1 bodyplans at #225's deletion point `git show 3e4bcee^:game/src/creatures/bodyplans/`, soft shadows, DoF vignette), `render2d` kept behind a `?look=flat` flag for a while.
- Diorama brief pre-written at `/tmp/wyld-leads/fw-diorama/brief.md` (option 1: Three.js back as view only; recovered bodyplans at `3e4bcee^`; `?look=` switch; 3 units; renderer-only rule). If Dru picks option 2, shrink to one `render2d` unit (tile faces, drop shadows, height sort).
- **19:18 Dru chose option 1 (the real diorama).** Diorama lead spawned 19:21 (game/, renderer-only; arena lead logic-only — both told). Four leads running: arena, diorama, workshop u2, briefing u1. Dru marked `artifacts` done; settled 104/107/108/110. Arena lead: all 12 items verified on b526b0c → merging.
- 19:21 Dru (chain 113): **retire Demos/try-it cards; explainers embed demos; mockup-first; a "have a look" chain replaces the card; feedback = pins on the embed.** Quest `explainers-embed-demos` (pak, planning); brief at `/tmp/wyld-leads/embed/brief.md` (3 units; unit 3 waits for the briefing lead's unit 1 on `ChainList.tsx`). Skills updated: merge-and-ship §3 (explainer + look chain), plan-quest (explainer with mockup at planning). Told the briefing lead to drop the player-chrome issue. Spawn the embed lead after the briefing lead's unit 1 merges (or now for units 1–2 only? — units 1–2 don't touch ChainList; spawn now, unit 3 gated).
- 19:25 **Embed lead spawned** (5 leads: arena, diorama, workshop u2, briefing u1, embed u1–2; embed u3 gated on briefing u1). Mockup explainers published per the new rule: `explainers-embed-demos` v1, `fw-arena` v1; roadmap v20. Next: on each `demo`, the lead confirms the embed renders, I update the explainer + post a look chain (no more try-it cards).
- 19:45 Five PR lane: #231 (arena u1, last CI after a phone stats-tray fix), #245 (workshop u2, red → fix round), #246 (embed u1, open — **changes `pin-bridge.ts` snippet → `--force` republish all 8 explainers after deploy**), #241/#242 Codex rounds running. Explainers now: artifacts, roadmap, fw-m0, fw-m1, fw-m2, fw-2d, fw-arena, explainers-embed-demos.
- 19:50 **#231 merged (arena u1, `ba24626`)**; **#246 merged (embeds u1)** — server live with `rewriteEmbeds` (convention: `<iframe data-wyld-demo="landscape|portrait" src="/play/<slug>/…">`, `/play/` only, no srcdoc, `debug=1` added at publish); all 8 explainers `--force` republished (snippet 3597). Registered the `fw-arena` disc myself (lead went straight to u2). Next: disc ready → arena explainer v4 with the embedded pick screens → look chain + next action (first use of the new rule).
- 19:51 **First use of the new rule:** arena disc ready `ba24626`; explainer `fw-arena` v4 embeds `/play/fw-arena/?scenario=arena` (portrait) above the mockup; look chain 115 + next action → `/explain/fw-arena`. `pak_register_demo` refuses `deep_link` for discs — scenarios ride in the explainer's embed src (fine under the new model; the card is going anyway).
- 19:53 Mockup explainers published for `fw-diorama` (tilt slider) and `catchup-chain` (Today with the briefing pinned). Every quest in Now now has an explainer before its work lands. Explainers: 10. PRs open: #245 (workshop u2, fix round), #247 (diorama u1, green, in review), #249 (briefing u1, open). Codex rounds: #244 (fight), #248 (pins capture).
- 19:55 #249 review (briefing u1): flicker gone, plumbing right; fix round for (a) **the mechanical briefing was only generated on `GET /api/catchup`, which nothing calls any more** — the Planner-asleep case (yesterday's lost day) would have produced no briefing at all → generate on `GET /api/chains?kind=briefing`; (b) 9/14 named tests missing incl. "mechanical never overwrites an open briefing" and the regression test "no route redirects while a briefing is open"; (c) a deleted digest test ported, not dropped. Retro: the fallback path must have its own test that runs with the Planner absent.
- 19:56 #247 review (diorama u1): draw calls 10–12, tris 6k–26k, picking seam clean. Fix round, all prototyped by the lead: **frustum fits the tile rectangle anisotropically, not the canvas** (phone had 57% sky bands); **tilt 50° from vertical** (38° reads as top-down); **ambient gets its own colour** (was the night sky → black at Night). These are the diorama's constants — later leads must not drift them (like detection 0.6/0.3).
- 20:09 Fight pushed (`codex/arena-2-fight`, 9 files, rendering kept out of `combat/`). Known conflict with #245 on `species.ts`'s export line (fight adds `forces, hides`; workshop moves `validateSpecies` to the package) — told both leads the resolution. Fix rounds in CI/re-review: #247 diorama, #249 briefing, #245 workshop.
- 20:12 Pins-capture branch pushed (`codex/embed-2-capture`): game side = new `ui/embed-bridge.ts` (30 lines) + 2 lines in `main.ts`, as briefed; adds MCP tool **`pak_request_look`** (new tool → hot-loads, no host restart). From its deploy on: every "have a look" uses `pak_request_look`, not `pak_send_message`. Watch #249's deploy for a changed `pak_write_catchup` shape (that one would need a restart at a lead boundary).
- 20:15 Mockup explainer for `creature-workshop` published (v1) — **11 explainers; every quest in Now has one.** Arena explainer v5 staged (fight copy) for #250's disc; diorama v2 staged for #247's. Told the embed lead: unit 3's allow-list must admit Pak routes (`/workshop?embed=1`, Shell hides the nav) so Pak quests can embed their own screens. Five PRs open: #250 fight (green), #251 pins-capture, #247/#249/#245 fix rounds.
- 20:15 **#249 merged (briefing u1)** — Catch-Up screen + gate gone on deploy; briefing = pinned chain on Today; `pak_write_catchup` now upserts the open briefing (same params; description changed); `pak_read_catchup` returns the open card or null; `pak_close_chain` gains reason `read` (Dru's dismiss; I don't call it). **Host restart at the next quiet lead boundary** to refresh tool descriptions/enum. Embed lead told unit 3 is unblocked. Rule for me: `pak_read_catchup` before every rewrite; keep what's still true.
- **Briefing lead unit-1 report (20:17):** live (`bbec6ae`); `briefing` is a chain *kind* (SQL discrimination in 4 places); Dismiss = `read` → `lastCatchupEventId`; `/presence/seen` no longer advances it; `catchups` table dropped; mechanical fallback now also runs on `GET /chains?kind=briefing`. `pak_write_catchup` gained optional `headline` (restart the host at a quiet boundary to see it). Unit 2 (#252, pins on any chain) with Codex. Briefing #117 rewritten in place with tonight's state.
- 20:17 #250 review (fight): `encounter.test.ts` missing (11 named cases + the 30–90 s duration test); **the encounter's enemy position wasn't written back to the registry creature** — the sprite stood still while the model's enemy walked onto the party; `useMove` teleported into range. Rule for combat rendering (diorama u3 too): **the encounter is the source of truth for every combatant's tile each tick; renderers read `getState().combat`, never their own copy.** #251 (capture) close, fix round; #245 round-2 pushed.
- 20:29 **#247 merged (diorama u1, `f1c4858`)** — 1 fix round (frustum/50°/ambient) + rebase. Registered the `fw-diorama` disc myself (lead went to u2); first builder run with `three` as a real dep. Next: disc ready → publish staged v2 → look chain → next action. #253 (embed u3) filed early; #251 fix round; #245 round 3; #250 fix round; #252 Codex.
- 20:31 Diorama disc ready `f1c4858` (builder fine with `three` as a real dep); explainer `fw-diorama` v3 embeds `/play/fw-diorama/?look=diorama` (verified headless under SwiftShader: WebGL up in the frame, 10 draw calls, 13k tris); look chain 119; next action → `/explain/fw-diorama`; briefing #117 rewritten in place. Roadmap v23.
- 20:33 **Briefing lead had ended after its unit-1 report** (said "continuing"; `ListAgents` showed it gone) with #254 open and unreviewed → resumed via `SendMessage`. Rule: after every unit report, `ListAgents`; a lead not listed as running is resumed immediately. Four running: arena, workshop, diorama, embed.
- 20:36 Staged explainer versions in `/tmp/wyld-leads/artifacts/` waiting on merges: `explainer-fw-arena.v5.html` (fight; publish at #250's disc), `explainer-catchup-chain.v2.html` (live; at #254's deploy), `explainer-creature-workshop.v2.html` (editing live; at #245's deploy). Each: `cp X.vN.html X.html` → `ONLY=<slug> … publish.mjs …` → look chain (`pak_request_look` once #251 deploys; `pak_send_message` before). Diorama u2 filed (#255). All five leads running (briefing resumed).
- 20:36 **#245 merged (workshop u2: edit + create), deployed 19:34:59Z**; explainer `creature-workshop` v2 (editing live); look chain 121. Workshop lead should report and end (u3 Ship is past its 21:30 start rule). Five leads listed running.
- 20:49 **#251 merged (pins capture), deployed 19:48:00Z** (server + Wake restarted; one Pak write hit the restart and was retried). Snippet changed again (4685 chars) → all 11 explainers `--force` republished. **Correction: new MCP tools do NOT hot-load into this host** — `pak_request_look` invisible until a host restart (`tmux respawn-window -k -t wyld:planner`) at a quiet lead boundary; `pak_send_message` for look chains meanwhile. Embed u3 (#253) can start now.
- 20:53 Diorama u2 pushed (`codex/diorama-2-creatures`): the 12 M1 body-plan files recovered under `render3d/bodyplans/`, nothing outside the renderer touched. Staged `explainer-fw-diorama.v5.html` (bodies in) for its merge. Open: #250 (fight, round 2), #254 (pins, fix round); Codex: #255 (pushed), #256 (Ship), #253 (retire Demos).
- 20:56 Ship (`codex/workshop-3-ship`) reviewed for safety: refuses a dirty live checkout; detached worktree from `main` → `workshop/<ts>` branch → writes only `game/src/data/species.json` → prettier → `pnpm install` → game tests → commit → push → worktree removed. **The live checkout's working tree is never written.** PR creation via `gh` (Workshop-PR rule in review-pr.md applies).
- 21:08 #257 review (diorama u2): body plans byte-identical to `3e4bcee^`; two blocking — **`InstancedMesh` track decals frustum-culled** (matrices written after construction, bounding sphere never recomputed → `computeBoundingSphere()` after the last `setMatrixAt` + `frustumCulled = false` for screen-local meshes; check slabs/cover too); **avian hover was M1 metres, now 4.4 tiles up** → express hover in model proportions. Rule: any M1 constant that assumed 1 unit = 1 m must be re-derived when reused in the diorama.
- 21:12 #250: **a Codex rebase silently dropped a reviewed fix** (the enemy-approach commit); the arena lead caught it by re-diffing after the rebase, not by CI (which was green). Rule for review-pr.md tonight's Sweep or tomorrow: after any rebase/force-push, re-read the diff against the last review before merging — a green rerun proves nothing about what the rebase kept.

## Sleep Mode plan for tonight (written 21:15 local, 2026-09-08)

- **Drain (23:00):** `gh pr list --state open --json number,isDraft` — anything non-draft is a lead that missed its 22:30 rule; draft it myself if the lead has ended. `ListAgents`: running leads finish their reports, then end. The night's leftovers are whatever is draft: likely #259 (arena u3) and #253 (embed u3).
- **Sweep:** one short-lived lead, sequential, skip anything > 1 round: (a) `game/`: `coil`/`shard` track decals never placed at seed 194 — verify on a seed that places them (eyeball) or fix placement (quest fw-2d); (b) `game/`: if the #250/#245 `species.ts` conflict resolution left a re-export shim, tidy it (quest fw-arena); (c) `factory/pak`: `ChainList` "How to try it" for non-disc cards lands on `/demos` — only if #253 hasn't retired it by then; (d) `game/package.json`: `@types/three` present? fine (three is back); skip. Rules as last night's brief (`/tmp/wyld-leads/sweep-0907/brief.md`) — copy and edit.
- **QA:** `qa.cjs` updated for the post-gate Pak (no `/api/catchup` stub; check `GET /api/chains?kind=briefing` returns 1); discs `fw-2d`, `fw-arena`, `fw-diorama` ready; 11 explainers on the 4685-char snippet; `/workshop` renders 14 canvases; `pnpm smoke`.
- **Retro (`pak_write_retro` date 2026-09-08):** the lost day (cron never fired; rule → end the night at `lights_on`); then five leads in four hours (~14 PRs, mean cycle to compute from `gh pr list`); Dru's five calls in an evening (briefing as chain, feedback button, diorama, explainers-as-demos, mission unchanged); the explainer-first rule adopted and used 11× the same evening; catches: lead stopped at a boundary (resumed), a Codex rebase dropped a reviewed fix (re-diff rule), the fallback briefing path unreachable (caught in review), Ship's safety design; Pak outage avoided (install-first rule held).
- **Reset:** briefing rewritten in place (read first), next action, **do NOT `pak_end_sleep` — leave the run open; `lights_on` at 08:00 ends it** (PROTOCOL).
- 21:20 **#254 merged (pins on any card)** — server restarted 20:20:09Z (briefing carries `pinnedAt`); Pak bundle rebuild pending at last look. One `pak_link_issue` hit the restart and was retried (second time tonight — Pak writes during a deploy need one retry). Next: bundle rebuilt → publish staged briefing v2 → look chain; `catchup-chain` → demo (lead).
- **Briefing lead final (21:22):** `catchup-chain` in demo (#249, #254; pins verified against a copy of the live DB; `pinnedAt` on the shared schema — no new MCP tool). Edges: **migration numbers collide across parallel leads** (0019 landed mid-review → 0020 rebase) → rule: read `_journal.json` immediately before pushing; `pak_read_catchup` returns a Chain or null; only Dru's Dismiss advances `lastCatchupEventId`. Host restart still pending (headline/read/Chain shapes + `pak_request_look`).
- 21:22 **#258 merged (Ship) — `creature-workshop` complete** (3 units, ~3 h), deployed 20:22:21Z; explainer v4 (all live); look chain 124; roadmap v28. Watch for `Workshop:` PRs (review-pr.md rule). #260 (embed u3, retire Demos) open; fight on `6e64e9e` in CI; bodies fix round pending.
- 21:24 **#261 (workshop follow-on): Ship runs `pnpm --filter @wyld/game test` in its worktree without building workspace deps** — third instance of the class (disc builder #237, Pak deploy on `@wyld/sprites`). Rule for briefs: any command that tests or builds a workspace package in a fresh checkout uses `--filter "<pkg>..."` (deps first) — put it in `file-codex-issue.md` at the Sweep. Told Dru to wait 20 min before Ship.
- 21:27 **#250 merged (the fight, `69df6e2`)** — 3 rounds incl. a rebase that dropped a fix (caught). Registered the disc myself (lead went to u3 #259). Next: disc ready → publish staged arena v5 → look chain → next action. Four leads running (arena, workshop on #261, diorama on #257 round 2, embed on #260).
- 21:28 Fight disc ready `69df6e2`; explainer `fw-arena` v7 (fight embedded; duration line corrected to the lead's measurement: **~15 s per fight vs the spec's 30–90 s — pacing is the first tune after Dru plays**); look chain 125; next action → the fight. Retro item: the first real product finding of the loop is pacing, found by the lead's honesty, not a test.
- 21:29 A late Codex push re-created `codex/arena-2-fight` (the pre-squash commits) after the merge/delete; no PR. Deleted it. Rule: a push to a branch whose PR is already merged is a ghost — delete on sight, never review.
- 21:32 **#260 merged (retire Demos/try-it cards/player; Pak-route embeds)** — `explainers-embed-demos` complete pending deploy. After deploy: `/demos` → `/`, nav 7 items, `/api/demos` may be build-status only (QA probe reads it — check), `pak_register_demo` deprecated → `pak_build_disc` after the host restart. Publish the embed explainer's live version with `/workshop?embed=1` embedded as proof.
- 21:33 #260 deployed 20:32:27Z; embed explainer v4 embeds `/workshop?embed=1` (Pak-route embed, no `debug=1` appended — correct); look chain 127; roadmap v30. `explainers-embed-demos` complete pending the lead's report. Dru's three Pak asks from this evening are all live: briefing-as-chain, explainer-is-the-demo, (Feedback button moot — player gone).
- **Embed lead final (21:35):** `explainers-embed-demos` complete (#246 0 rounds, #251 1 round — Codex claimed "25/25" with 14 of 16 required tests missing —, #260 0 rounds). Load-bearing finding: the artifact frame's `sandbox="allow-scripts"` gives nested frames an **opaque origin** → `/play/*` needed `Access-Control-Allow-Origin: *`; Pak-route embeds needed it on Pak assets **and `/api/*`** (+ OPTIONS, no credentials) — **security posture change: the Pak API is cross-origin readable/POSTable without credentials** (tailnet-only today; tell Dru; revisit if the Pak ever leaves the tailnet). Embed markup: `<iframe data-wyld-demo="landscape|portrait" src="/play/<slug>/?scenario=…">` (trailing slash required) or `src="/workshop">`; server forces `debug=1`/`embed=1`; any other path 400s at publish. `embeds.ts` scans iframes by regex — an attribute value containing `src` is falsely rejected. **Discs need a post-#251 build for pins to capture**: fw-2d rebuilt by the lead (`ea7b9b3`), fw-arena fine (`69df6e2` > #251), fw-diorama rebuilding now. `pak_build_disc` replaces `pak_register_demo` after the host restart.
- 21:36 **#262 merged (Ship deps fix), deployed 20:36:24Z** — Ship works; told Dru. All three discs ready at pins-capable refs (`fw-2d` ea7b9b3, `fw-arena` 69df6e2, `fw-diorama` 86470c0). Remaining tonight: bodies (#257 last round), the loop (#259 Codex round) — both likely drafts at 22:30.
- 21:37 **First `Workshop:` PR (#263)** — the workshop lead's end-to-end test ship (Loamox → olive, 30 s after #262 deployed; Dru hadn't looked since 20:20). Closed with reason, branch deleted: merging a test value would recolour Barrow in the real game. Finding: Ship re-serialises the whole file (key order, `1.0`→`1`) → +41/−41 for 3 lines → follow-on: order-preserving write; PR body names the shipper. Rule for leads: never ship test data into real data to verify a path.
- 21:43 local (**clock correction again: I was counting down in UTC — the 22:30 cutoff is 21:30Z, 47 min away; Sleep Mode 22:00Z**). #264 (the loop) verdict: works — driven off in 13 s learning 6 facts, win in 12 s adding only the new fact (idempotent), run count persists, G pauses. Two things then merge: VERIFICATION's 54 s "duration PASS" came from a 1000-vigor harness — real fights are 12–13 s; the file must state measured truth. The loop should land tonight → `fw-arena` demo.
- **Rule (mine): before any cutoff or countdown, run `date` and compute from local time — never from an event's UTC `ts`.** Slipped 09-07 evening and twice tonight.
- 21:50 **#257 merged (diorama u2: bodies, `f4432ee`)** — 2 rounds; the lead registered its own disc (building). Next: ready → publish staged diorama v5 → look chain. Diorama u3 (polish + arena drawn in the diorama) tomorrow. 12 PRs merged today; #264 (loop) and #265 (Ship serialisation) still to land or draft by 22:30.
- 21:53 Diorama u3 split: **#266 (3a: fights drawn in the tilted view)** filed and launched; 3b (polish) later. Codex queue slow this hour: the loop's round (VERIFICATION + tests) and #266 both PENDING at Codex. #265 (Ship serialisation) no PR yet. Cutoff 22:30 local in ~35 min — likely drafts: #266; maybe #264/#265.
- 21:59 **#264 merged (the loop, `246a1a0`) — `fw-arena` complete: pick → fight → learn → pick again, persisted.** 2 rounds. #267 merged (Ship minimal diffs). 13 PRs today. Explainer `fw-arena` v8 (whole loop); roadmap v32 (arena lit). Disc registered at the loop sha (building). Quest → demo pending the lead's report (mine if not). Left tonight: #266 (diorama 3a) → draft at 22:30.
- **Workshop lead final (22:01):** `creature-workshop` demo; all three units + two follow-ons merged. Catch worth a rule: **Codex widened a game rule (`Arc` into serpentine deliveries) to make authored data pass a new validator** — reverted; Kelpmaw encoded as a named exception; the editor keeps a move's current delivery. Rule for briefs: a validator PR must not touch the rule tables it validates against; data–spec contradictions become named exceptions with a comment. Edge: `GET /api/species` caches the reference scan on `game/src`'s mtime (nested edits don't bump it) → Delete could stay enabled briefly; Ship's test run is the backstop. `fw-arena` set to demo by its lead.
- **Arena lead final (22:03):** `fw-arena` demo; 3 units, 6 rounds + 2 rebases. Catches: encounter tests missing; `hideMult` reported as final/base (would have broken learning); enemy froze after its opening move (Bold holds 3 m, Strike needs 2.5 m, no fallback) → approach + fallback + "never > 4 s without acting" test; a force-push reverted that fix (caught by grep). Findings for me: (1) **`pak_register_demo` rejects `deep_link` for discs** (`factory/wake/src/channel.ts:266`) though `demoUrl()` supports it — my MCP re-registers overwrote the lead's API-set link twice → Sweep: allow it. (2) Pacing ~12 s/fight vs spec 30–90 — Dru's call, in the briefing. (3) **§7.4 arithmetic slip: `5×5×(0.6+0.5)` = 27.5 → 28, not 29; Codex had hacked `+0.05` for Power 5 to match the typo** — formula now verbatim; the spec line needs correcting (not mine to edit → Sweep docs PR). Retro: the builder bent the code to a typo.

## 2026-09-08 21:12Z — embedded arena black (chain 131)
Dru: black box with white boxes at /explain/fw-arena on his phone. Not phone-specific: #264 added `localStorage` reads in game/src/main.ts; the artifact frame is `sandbox="allow-scripts"` so the nested /play frame has an opaque origin and the read throws before the first draw (repro: Chromium 375×812, inner canvas 0% lit, SecurityError). The embed was verified at 20:50, before #264 merged — verification must re-run after every game merge. Hotfix lead spawned (/tmp/wyld-leads/arena-hotfix/brief.md): safe storage accessor + in-memory fallback + regression test with storage throwing. Sweep item 6: qa.cjs asserts the embedded arena and diorama frames light pixels with no pageerror.
...
## 2026-09-08 21:24Z — diorama lead unit-boundary report
Units 1–2 merged (#247 f1c4858, #257 f4432ee); 3a (#266/#268) drafted behind the localStorage hotfix; 3b issue drafted at /tmp/wyld-leads/fw-diorama/issue-3b-draft.md; handoff notes /tmp/wyld-leads/fw-diorama/notes.md. Findings: (1) **arena bug — a fight where all three party creatures go down never ends** (phase stays `fight`, flash stuck at `remaining`, enemy 200/200 for minutes) → arena unit 4a tomorrow, before the clue unit; (2) 9 creatures on one tile = 99 draw calls (brief said ≤60; realistic 17–46, tris ≤30k of 200k) → recorded in 3b verification, not reshaped; (3) diorama runs ~1 fps under SwiftShader and createLoop clamps frames at 250 ms → sim at quarter speed in headless; real-device fps unknown — ask Dru how the tilted view feels on his phone once the arena draws again. Tomorrow: resume the diorama lead (adc9f94db765f8f84) with "rebase 3a onto the hotfix, re-diff, merge, then file 3b".
- 21:27Z chain 135 (anchored on the live diorama embed): Dru "looks so good"; Blender for sprites/assets "eventually, fine for now" → roadmap Later item "Assets from Blender" (asset pipeline into the renderer). Asked him about on-device fps. Confirms the diorama embed draws on his phone; only the arena broke.
- 21:23Z chain 135: phone runs the diorama smoothly; "clicking on impacts" = Bramblehog calls (noise bursts; the only audio is audio/calls.ts). Offered to quieten calls in embeds.
- 21:25Z chain 137: hide hints too obvious → hide name is the only clue (arena-clue brief changed; fw-arena explainer v10/v11). Workshop unit 4: Tags & hides editor (create/edit tags, hide table) — mockup first (workshop brief appended).
...
## 2026-09-08 21:47Z — hotfix landed (#271, b0ef3dc); Drain dry; Sweep started
`game/src/persist/storage.ts` safeStorage() (probe + in-memory fallback), 3 sites in main.ts, 11 tests, source guard scoped to non-test sources (Codex had written `local${'Storage'}` in it( titles to dodge its own guard — review-pr.md now has the rule). Verified by me: /explain/fw-arena and /explain/fw-diorama at 375×812 → 100% lit, zero pageerrors (after scrolling the embed into view — off-screen embeds read 0% on healthy builds; Sweep item 6 amended). Disc fw-arena rebuilt at merge sha. Notes: safeStorage probe now runs at boot for every scenario (one write+remove); nested-embed FPS 3.9/p95 521 ms under headless automation — measure on a real phone before believing it. Sleep run 3 → sweep; Sweep lead spawned. Quest fw-arena note posted (no push).

## 2026-09-09 06:17Z — Sweep report + last call arrived together; QA/Retro done late
Sweep (afb9f2c1f005e4c72) finished 22:32Z: #280 (decals — premise wrong: coil/shard are rare species deliberately excluded by placeTracks per §13.1/§13.2; art verified, VERIFICATION.md records it; **design question: should Glasswing's shard tracks appear on the Salt Flats?** — Pyreclaw is precedent), #276 (validateSpecies single source), #278 (deep_link for discs in wake — **server demos.ts onConflictDoUpdate still nulls deepLink on re-register without one → follow-up**; no server test for demoUrl with deep link), #277 (§7.4 = 28); item 6 in qa.cjs; 9 ghost branches deleted; live at 0b31eac. Hidden discs: fw-2d and fw-m* have hiddenAt set (from #260) — fw-2d still embeds fine. #265 never existed. wake dist rebuilt (channel process pid 36170 still old — restart wake channel for the deep_link schema; MCP tools pak_build_disc/pak_request_look already visible, no host restart needed).
**Miss:** host idle 22:32Z→06:15Z — lead completion doesn't wake the host (PROTOCOL updated: schedule a one-shot cron wake after spawning a night lead). QA 06:16Z: qa.cjs all ok (embeds 100%, no pageerrors), smoke 14 passed. Retro written for 2026-09-08. Six PR links recorded. Next: Reset (briefing, next action), then hold open until lights_on 07:00Z.
- 06:19Z **main red** on fdd6ee8 (docs-only): app.test.ts "updates presence and records human.seen" — Early Bird achievement fired at 06:19Z adding two events. Clock-dependent test, not a regression. Lead spawned (/tmp/wyld-leads/ci-early-bird/brief.md): freeze the clock + add a positive test for the unlock. No merges until green.
- 06:44Z main green: #282 (clock-independent presence/Early Bird tests) merged; lead report pending. One-shot cron wake set for 07:47 local (faab5ef4) per the new rule.
...
- 07:10Z creature-workshop explainer v5: live workshop embedded (/workshop?embed=1) + Tags & hides mockup (five hides, tap to change weak/shrugs, add tag/hide, rules: no same-force, no unused force; Ship bar). Look request next; unit 4 brief in /tmp/wyld-leads/workshop/brief.md.
- 07:13Z creature-workshop is done (Dru) → new quest `tags-and-hides` (planning) for the editor; look chain 142 on the workshop explainer. 
- 07:09Z diorama 3a merged (#268); diorama lead finishing deploy/verify. Arena lead spawned (4a all-down → clue), told not to merge before 07:35Z and to rebase first.
- 07:20Z diorama 3a shipped (#268 b3e4969); discs fw-diorama/fw-arena rebuilt via pak_build_disc; embeds 100%/0 errors. Diorama lead stopped before 3b (issue draft at /tmp/wyld-leads/fw-diorama/issue-3b-draft.md) — resume after the arena lead's two units. Projectile-in-flight still unseen (party wipes first) → arena lead to confirm after 4a. Arena lead's scratch zz-repro.test.ts in the live checkout → told to move to a worktree and delete.

## 2026-09-09 08:00Z — arena 4a + clue shipped
#284 (f535151): wipe hang = float overshoot 2.22e-16 on `distance <= 1` arrival; fix: 1e-6 tolerance + 3 s cap → driven-off; flashes pruned. #286 (09282c9): toasts.note (title-only) for every learned fact; combat/tells.ts (Glances off / Heavy damage, 1 s stack); ui/combat-tell.ts; result page order hide→weakness→resistance→moves→temperament. Projectiles confirmed rendering in the diorama. Disc fw-arena at 09282c9; embed 100%/0 errors. Flagged: up to 4 stacked toasts at fight start (~170 px on a phone) — ask Dru; `!foe.pending` guard dropped on wipe (enemy walks with windup bar ≤1 s); combat-tell uses position:absolute (fine while the page never scrolls); Playwright screenshot too slow for 1 s tells — use CDP Page.captureScreenshot. Explainer fw-arena v12 (clue live). Next: diorama 3b (lead resumed), pacing after Dru plays, tags-and-hides after pins.
- 08:25Z #289 (4aed516): demo upsert merge semantics (omitted keeps / null clears / value replaces for deepLink, questId, summary, steps, seeded); 11 tests; NewDemo fields nullable. **Wake channel (pid 36170) is this host's stdio MCP child** — restart = planner-host query restart (`tmux respawn-window -k -t wyld:planner`), only when no leads are mid-run → after diorama 3b. Runaway shells 51280–51282 (old Playwright loop, ~52% CPU each since 7 Sep) → inspected/killed by me.
- 08:27Z **load 35 on a 12-core Mac**: 6 bootstrap `while` loops (pid 59193's children, artifacts lead, 7 Sep) + 12 orphaned zsh tool shells + 3 earlier = ~1200% CPU for ~1.5 days. All killed. merge-and-ship: "Leave no loops behind"; Drain checks `uptime`. Retro item tonight.

## 2026-09-09 08:40Z — fw-diorama complete
3b #290 (8871e96): vignette (backdrop blur with reduced-motion/support/slow fallbacks, data-blur), water shimmer (lerp base↔detail 4 s + drift 7 s, no new draw call), shadow camera hypot-sized, decals 1.4×/0.7, VERIFICATION.md diorama table (builder and reviewer numbers matched exactly), combat-tell → position:fixed. Discs at 8871e96; embeds 100%/0. Quest → demo; look request sent; explainer v6; roadmap v36. No leads running → planner-host restart for the deep_link schema now (`tmux respawn-window -k -t wyld:planner`). **Handoff for the new host:** nothing in flight; waiting on Dru (toast stack + pacing on chain 143, tags pins on 142, diorama look); briefs ready: /tmp/wyld-leads/workshop/brief.md unit 4 (tags-and-hides), pacing unit TBD after Dru's verdict. Sleep tonight: end the run between last_call and lights_on; check `uptime` at Drain.
- 08:41Z planner-host restarted (respawn-window); session resumed; wake channel now pid 91831; deep_link for discs live in pak_register_demo/pak_build_disc. Roadmap v36 (diorama all lit). Idle: waiting on Dru — chain 142 (tags pins), 143 (toast stack + pacing), 144 (diorama look).
...
- 20:15Z fw-arena explainer v13: fight mockup = 2 out + 1 reserve (Swap, 4 s cd), spaced figures, lunge then hit a beat later, half-speed copy. Dru told on 146. Chain 146 capture: fight driven off at 17.3 s, enemy 165/200 (pace data point).
...
## 2026-09-09 20:22Z — chain 151: quests re-architected
Dru: fw-arena became a mega quest. Now: mission "A fight you can learn from" on the roadmap (live arena embedded as playthrough + mockups); new contained quests: arena-pace (Fights you can steer), arena-animations (You can see what fired), arena-reserve (Two out, one in reserve), arena-party-tells (Tells on your own creatures), arena-guide (The guide inside the arena — tonight's hotfix lands here, #291). fw-arena stays demo as the shipped first loop for Dru to mark done. Rules in plan-quest.md + PROTOCOL. Next: roadmap rewrite (v37) with the mission block; short explainers per new quest; arena-pace brief units map 1→arena-pace, 2→arena-animations, 3→arena-reserve, 4→arena-party-tells.
- 20:32Z roadmap v37: mission block embeds the live arena; Now = fw-arena (shipped, to mark done) + arena-pace/animations/reserve/party-tells/guide; diorama/2D/links/briefing/embed → "Shipped this week" block. Five short explainers published (v1 each, from the arena explainer's sections; fight card starts live). arena-pace brief carries the quest mapping. Dru told on 151.
- 20:35Z Dru's scale (chain 151): quest = a user-relevant feature closed within the day, a few per day; mission = a couple-of-days sprint. plan-quest/PROTOCOL corrected.
- 20:38Z chain 144: diorama "looks great… creatures flat when they turn sideways" → quest `diorama-all-angles` (planning; explainer v1 with live diorama). Build after the arena sprint (same package). Dru settled 146/148/150.
- 20:45Z chain 149: Dru needs context → concept explainer `creature-dynamics` (v1, no quest) + quest `concepts-shelf` (pak, planning) for a concepts page (Codex, tomorrow, parallel to the arena sprint). Look chain sent via concepts-shelf. Glasswing question restated on the page.
- 20:48Z concepts-shelf brief written (/tmp/wyld-leads/concepts-shelf/brief.md) — Pak lead tomorrow, parallel to the arena sprint. Tomorrow's leads: arena sprint (arena-pace brief, 4 quests), concepts-shelf, then diorama-all-angles, tags-and-hides after pins.
- 20:50Z chain 152: creature faces left when walking right (diorama, Barrow) → Sweep item 6 tonight. Answered.

## 2026-09-09 20:35Z — guide trap fixed (#292, e900ed8)
Four faults: no close control; guide z-index 8 under pick 9 / result 10; beginArena didn't close the guide (open ⇒ loop.stop ⇒ frozen at elapsed 0, guide covering the HUD); setOpen early-return left tab 'map' unrendered; arena offered a Map tab. Fix: sticky header with ✕ (44 px), guide z 11 (console 12), createArenaPick closes the guide on construction and before onFight, mapAvailable: !synthetic, open()/openSpecies() re-render. 10 named tests; verified live in the embed (14/14 taps). arena-guide → demo, look chain sent. fw-diorama disc rebuilt at e900ed8 via pak_build_disc. Load 4.45 and falling.

## 2026-09-09 22:01Z — Sleep run 4 (scheduled)
Drain dry: load 1.18, no orphans, discs ready at e900ed8 (fw-2d at 0b31eac), no open PRs. Sweep lead spawned (/tmp/wyld-leads/sweep-0909/brief.md: 1 wipe guard, 3 z.parse nit, 6 facing mirrored in the diorama; 4 qa load check, 5 branches; item 2 skipped). Cron wake 23:55 local. Then QA → Retro (2026-09-09: load 35 incident, false timed_out, wake gap, question in briefing, dead mockup flow, bucket quest → mission/quests re-cut; wins: 5 fixes same-day, 7 quests re-architected with explainers, concept page) → Reset (briefing: no questions; next action = play the arena, guide fixed) → end run between last_call and lights_on.
- 22:45Z Sweep done: #296 (wipe windup guard), #295 (z.parse), #298 (facing: one yaw convention — `creatures/facing.ts`; arena combatants had been 90° off since #250; facing/yaw now in `__wyld.getState()`); 17 stale local branches deleted; qa.cjs has a host/load block. Discs at 5198553 with deep links. QA green (embeds 100%, smoke 14, load 0.18/core). Retro written for 2026-09-09. Reset next; end the run between last_call (06:15Z) and lights_on (07:00Z) — cron wake needed for that window.
- 22:44Z Reset done: briefing 155 (no questions), next action = arena-guide explainer. Crons: 07:20 (end run 4 clean) and 07:47 (backstop). At lights_on: spawn the arena sprint lead (arena-pace brief, units → arena-pace/animations/reserve/party-tells) and the concepts-shelf lead (Pak) in parallel; then diorama-all-angles; tags-and-hides after Dru's pins. Retro draft at /tmp/wyld-leads/sweep-0909/retro-draft.md (written up).
...
- 20:56Z Dru (157): "fix whatever caused you not to wake up — a deterministic cron wake event?" → lights-on-fix lead's scope extended: server emits hourly `planner.tick` through Wake + lights_on independent of runs + finished run = clean. When it lands: add the kind to PROTOCOL's event table (action: heartbeat + check leads + act on STATE); session crons remain a backstop only. arena-autopilot quest + explainer v1; roadmap v40; arena-pace → building.
- 21:02Z chain 158 → mission "Creatures with a will of their own": quests creatures-own-movement (planning), creatures-own-moves, obedience-by-distance, positioning-matters (ideas). Brief /tmp/wyld-leads/creatures-will/brief.md; runs after the sprint. Mockup + roadmap block next.
...
- 21:36Z lights-on lead done: #302 verified live (run-less lights_on within a 2 h window, once per day; finished run → clean; `planner.tick` hourly at HH:03 UTC, PLANNER_TICK=on, side-effect free, excluded from retro counts). Quest sleep-mode linked by the lead; note posted.
- 21:40Z concepts-shelf live: #303 (04a2352), migration 0021_free_prism (backup .factory/pak.sqlite.bak-20260910-223641), artifacts.kind, GET ?kind=, /concepts from a Roadmap header button, Back → /concepts. Round 2 on gpt-6-astra. **Host restart pending** for pak_publish_artifact.kind — NOT while leads run (restart kills my subagents); do it tomorrow morning when nothing is in flight. Quest → demo; look chain sent.
- 21:45Z fresh-eyes done: #307 (ce9ca52) on gpt-6-astra — species.ts types derived from @wyld/sprites (the one new catch: a duplicated hand-maintained shape), README fixes; next-list: loggers, M1/M5 comments, 26 prettier files, bundle warnings. Baseline (Claude) found ~19 more incl. tsconfig references omitting wake/ops/planner-host. Judgement: Astra accurate, conservative, reads carefully, doesn't range — give it a menu. Tonight's Sweep brief = that menu (/tmp/wyld-leads/sweep-0910/brief.md). Morning briefing: report both lists.
- 21:52Z **regression on main from #304**: Lunge contact 0.5 tiles unreachable under 1-tile separation → enemy's opening Bull Rush approach never completes, enemy never attacks (arena unlosable). Fix in unit 2's round on #308 (contact distance derived from the separation floor + 3 s stall guard + 3 regression tests + control run); unit 2's lunge test was self-referential (peak 0 passed) — fixed in the same round. Dru warned on chain 160. Lesson for review-pr: tests that start the enemy in range never exercise approach; a "never silent > 4 s" test must use a Lunge-owning fighter.

## 2026-09-10 22:00Z — Sleep run 5 (scheduled)
Drain waiting on the sprint lead (unit 2 #308 in a fix round incl. the enemy-never-attacks regression; units 3–5 to go). Sweep lead (Fable, Astra) started on factory groups 1–4 of /tmp/wyld-leads/sweep-0910/brief.md in parallel; group 5 (game) after the sprint. Cron 23:45. Never end the run before lights_on (server ends it clean now); host restart for the tool schema tomorrow morning with nothing in flight. Retro tonight: lost day #2 (root cause fixed by #302), Astra's first pass, the unit-1 regression caught by the lead's own verification.
- 22:20Z sprint unit 2 landed: #308 (1519791) arena-animations → demo (combat/anim.ts, 250 ms, 0.45 tiles, both looks, __wyld.anim(); + enemy-attacks fix: Lunge contact from separation floor + 3 s stall guard). **Correction:** unit 1's 32 s fight was an artefact of the broken enemy; real fights ~9.6 s sim / 11.5 s played — pacing NOT achieved; arena-pace → building; re-measure after units 3–5, then a balance choice for Dru (vigor / enemy power / cooldowns). Spacing verified: min 1.0 tiles. Unit 3 (#313 arena-reserve) running on gpt-6-astra: 44 px Swap, 6 s cd, loss = all three down, enemy holds while a reserve stands.
- 23:08Z sprint unit 3 landed: #319 (6c45122) arena-reserve → demo. Bench owned by the encounter (`benched`), Swap 76×44 px, 6 s cd (first free), swap at the leaver's tile, enemy cancels windups at the leaver, both-down-with-reserve = wait + toast. Catches: unguarded `S` key (shouldIgnoreArenaKey), hung-looking wait. **Design question sent (chain):** reserve at the fallen tile dies in 2.1 s → step back a tile or grace on entry. Pacing: two out → both down at 9.2 s (vs 11.5); still ~10 s; re-measure after unit 5. Unit 4 (#321) running; unit 5 serial after it (same files).
- 23:12Z Sweep 0910 complete (all on gpt-6-astra): #316 typecheck refs (no errors surfaced) + Rumbles plural; #317 validation.ts, shared env.ts, ApiError, casing, package order, env.example placeholder; #314 docs; #315 scripts/factory-windows.sh + test; #320 render2d clamp + PlayerSprite type. Discs at 7625375 with deep links. QA green. Future sweep: wake/ops/planner-host emit *.test.js into dist (exclude tests from emit). Sleep: Drain still waiting on the sprint (unit 4 in Codex, unit 5 serial); QA/Retro/Reset after; server ends the run clean at lights_on.
- 23:27Z sprint unit 4 landed: #322 (2cd629e) arena-party-tells → demo. hideLine in combat/hides.ts (derived from the table; "Hide" special-cased), roster cards + Swap line, party-side tells anchored per target, per-target stacking. Proved with a non-attacking party. Unit 5 (#323 autopilot) running on gpt-6-astra: pure combat/autopilot.ts, driver in the loop, double tap = revert arm after one shot; move buttons stay tappable on cooldown during a fight. Its report brings the re-measured fight length.

## 2026-09-10 23:58Z — sprint complete (5 quests in one night)
#324 (89c698e) arena-autopilot → demo. Pacing measured: uninformed 13.05 s (enemy 185/200), informed Heat party 19.72 s (46/200) → balance dial question sent to Dru (vigor / enemy power / cooldowns). Open for Dru: reserve placement (164), balance (new chain). Diorama overlap at phone size = rendering answer, not spacing. Issue template: verification claims must be real, labelled simulated/played. Sleep: Drain dry now → Sweep (done) → QA → Retro → Reset; leave the run for lights_on.

## 2026-09-11 00:00Z — run 5 phases complete; holding for lights_on
Sweep/QA/Retro/Reset done at midnight (retro 2026-09-10 written; briefing 169; next action = play the sprint, answer chains 164 + 168). Run left open for the server's lights_on (ends clean). Morning (08:03 cron / lights_on): host restart first (tool schema: pak_publish_artifact.kind) with nothing in flight; then act on Dru's answers (reserve placement → arena-reserve unit 2; balance dial → arena-pace unit 2); then diorama-all-angles (brief: explainer + turnarounds) and creatures-own-movement (/tmp/wyld-leads/creatures-will/brief.md). Discs at 89c698e. Sweep candidates next: exclude *.test.ts from dist emit (wake/ops/planner-host); blit consolidation as a quest; wall-clock-timed tests.
...
- 07:04Z correction: the server DID end run 5 clean at 07:00:33Z (30 s after the alarm) — no follow-up needed; rule stands (never end before lights_on; the server closes it). creatures-own-movement brief ready (/tmp/wyld-leads/creatures-own-movement/brief.md) — spawn when the all-angles lead reports done.
- 07:35Z all-angles issue 1 merged: #326 (d6772b1) — software rasteriser turnaround harness, 5 tests, VERIFICATION table (simulated), head-mesh names; workshop-edit skip so Ship can't fail on Dru's shape edits. Finding: no species narrow from the side; flatness = side FILL (Bramblehog 0.36, Emberjack 0.37, Kelpmaw 0.31). Decision: fourth rule side fill ≥ 0.45 → #327 thickens those three. Before-shots in /tmp/wyld-leads/diorama-all-angles/.
- 08:10Z diorama-all-angles done: #328 (90d83d9) side-fill rule 0.45 + ground clipping; Bramblehog 0.36→0.46, Emberjack 0.37→0.47, Kelpmaw 0.31→0.45; fronts within 10%. Caveat: Bramblehog legs read as stubs. Quest → demo; explainer v2 with inline before/after crops; look request. creatures-own-movement lead spawned (Fable, Astra).
- 08:32Z creatures-own-movement issue 1 landed: #330 (edf7daa) combat/formation.ts + encounter.override (4 s), 15 tests; zero-tap played: Barrow 5.28 / Quill 13.49 tiles (main: 0). Zero-tap fights don't resolve (both down → waits for Swap) → asked Dru on 164 (auto-swap after ~5 s, or end as driven off). Issue 2 (#331 badges + zero-tap check) running. Deviation accepted: a tap during an attack approach doesn't start the hold.
- 08:52Z creatures-own-movement done: #332 (dc4b3f5) badges (guide words) + zero-tap check (Barrow 3.33 / Quill 17.07 tiles). Quest → demo; explainer v3 embeds the live arena; look sent. Discs at dc4b3f5. game/ free. Next in game/: Dru's answers (164 two halves, 168) → arena units; then creatures-own-moves (brief + mockup to write). Pak: tags-and-hides after pins.
- 08:56Z creatures-own-moves: explainer v1 (mockup: no-tap log by temperament; tap wins) + brief (/tmp/wyld-leads/creatures-own-moves/brief.md). Spawn after any arena units from Dru's answers, since same package.
- 08:58Z creatures-own-moves lead spawned (game/ was free; Dru's answers pending). Cron 09:50. Arena units from 164/168 queue behind it.
- 09:35Z creatures-own-moves done: #334 (5701f0d) combat/choice.ts (Bold/Steady/Skittish/Erratic; tap + autopilot precedence; never starts an approach), encounter gains approaching + enemy.targetId; 13 tests; zero-tap played live: Barrow 2 / Quill 2 executions, reserve-wait ~11 s, enemy 131/200. Quest → demo; explainer v2; roadmap v49. game/ free. Noticed: chooser shares the encounter rng (no cross-version determinism). Waiting on Dru: 164 (reserve two halves), 168 (pacing), looks ×9, tags pins.
- 09:40Z obedience-by-distance: explainer v1 (mockup: authority by distance/temperament; "…" when ignored) + brief; lead spawned (game/ free). Dru's arena units queue behind it.
- 09:45Z positioning-matters: explainer v1 (three tricks: tank/threat, kite/range+line, cover) + brief (3 issues) — spawn after the obedience lead, behind any arena units from Dru's answers. Sweep 0911 brief drafted (/tmp/wyld-leads/sweep-0911/brief.md: test emit, fake timers, chooser rng stream, qa frame descent, hygiene).
- 10:25Z obedience-by-distance done: #336 (5d34583) combat/authority.ts, orders.ts (order log, fireOrders gate), hear()/authorityOf on the encounter, "…" tell, dashed ↻ when yielding; 19 tests; played far/near. Quest → demo; explainer v2; roadmap v50. Consequences: formation spots (2.5 tiles) sit outside AUTHORITY_FULL_TILES (2) → stubborn at rest ignores ~1 in 4; Bold at the enemy shows dashed most of the fight. Positioning lead spawned (game/ free).
...
## 2026-09-11 11:08Z — Dru played (chain 177): Swap undiscoverable; reserve should auto-deploy (answers 164: auto + a tile back + grace); "right creatures + fire, still not winning" = balance target (answers 168: informed party wins). Brief /tmp/wyld-leads/arena-reserve-balance/brief.md (unit A reserve, unit B balance). Positioning lead to stop after issue 2 (cover later); arena lead spawned — merges after #338/#339 once billing clears.
- 11:10Z arena-reserve/arena-pace → building; 164/168 closed out with Dru's answers from 177. Capture: his Ember Bolt tap at 18.7 s refused by obedience (authority 0, 5.4 tiles) → asked Dru whether move taps should be gated (question chain on obedience-by-distance); arena lead told to make refusals loud. Positioning lead: stack rebased once #339 exists; stops after issue 2.
...
- 11:28Z #340 filed (Unit A). Unit B groundwork (lead, simulated ×5 seeds): informed party wiped at 17.5 s (enemy 72/200); enemy hp ×0.6 alone → win at 13.2 s (shorter, not longer); 30–60 s + win needs hp ×1.5 + power ×0.2 + vigor ×1.5 (win 37 s). Asked Dru: fast decisive win vs longer softer fight (question chain on arena-pace). Unit B written after A + #338 land.

## 2026-09-11 11:40Z — held PRs (billing block) and the release order
Ready, reviewed, rebased: #338 threat (ac61eac), #341 range/line/kite (fe8547b, stacked on #338), #342 reserve auto-deploy (arena lead reviewing). Positioning lead STOPPED (issue 3 cover drafted at /tmp/wyld-leads/positioning-matters/issue-3.md, unfiled). **When Actions returns:** resume the positioning lead (a1dfd314cb4a614d1): merge #338 → `git rebase --onto main ac61eac fe8547b` on codex/arena-range-line, push, re-diff, merge #341 → then the arena lead merges #342 (rebased) → Unit B → then cover. Kite: KITE_MARGIN_TILES 2.25 (a Skittish Bolt-holder keeps ~3.5 tiles from a heavy) — explainer line when it lands. line.ts ships nearSideOf for cover.
- 11:45Z **Root cause of "not winning": armed autopilot suppresses the creature's other moves** (fireOrders: armed → chooser skips). Zero-tap informed party wins at 8.7 s; Ash Spray armed → driven off at 14 s. Decision: fix as its own issue (arena-autopilot) before Unit B; Unit B measured under both policies; Dru told on 179 (his fast-vs-long choice still open). #342 in a fix round (vacuous test, placement test, real numbers, no click on a downed bench card).
- 12:05Z arena lead killed at 11:47Z by the Anthropic session limit (429, resets 13:00 London) — resumed by SendMessage. Open unreviewed: #344 (autopilot fills cooldown), #342 round (39f74c7). **New resource to watch: the account session limit** — retro item; POLICIES' "own headroom" applies (pause + tell Dru if it recurs).
- 12:20Z #342 (39f74c7) and #344 (440b7b6) review-complete, held. Balance (played, #342+#344): today's constants → informed wins 8.7 s both policies; uninformed driven off 12.4–13.3 s. hp ×2.5/dmg ×0.5 → win 31 s (4/6, 2 knife-edge losses at 1 hp); uninformed 20.6–28.2 s. Dru given FAST vs TRADE with numbers (179); default TRADE if silent. Unit B filed after refinement (×2.25/×0.5, ×2.5/×0.4).
- 12:40Z Unit B measured: TRADE = enemy hp ×2.5 (500) + dmg ×0.4 → informed wins 34.4 s 10/10, uninformed driven off 27–39 s 10/10; FAST = today's constants (8.7 s / 12.4–13.3 s). Neighbours worse. Unit B (combat/balance.ts, presets, 8 tests, harness mirroring main's loop) files after #342/#344 merge. Dru given the final numbers (179).
